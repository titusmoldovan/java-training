...
public class Main {
    public static void main(String[] args) {
        ...
        Actor actorOscar2018 = new Actor("actor cu oscar din 2018", 55, new Premiu[]{oscar2018});
        ...
        for(int i = 0; i < studioDatabase.length; i++){
            if(studioDatabase[i].filme.length > 2){
                System.out.println("Studio cu cel putin 2 filme: " + studioDatabase[i].numeStudio);
                }
            }

        for(int i = 0 ; i < studioDatabase.length; i++) {
            for (int j = 0; j < studioDatabase[i].filme.length; j++) {
                for (int t = 0; t < studioDatabase[i].filme[j].actori.length; t++) {
                    if (studioDatabase[i].filme[j].actori[t].nume == "actor cu 2 oscaruri") {
                        System.out.println("Studio cu actor cu 2 oscaruri: " + studioDatabase[i].numeStudio);
                    }
                    if (studioDatabase[i].filme[j].actori[t].varsta > 50){
                        System.out.println("Film cu actori batrani: " + studioDatabase[i].filme[j].numeFilm);
                        break;
                    }
                }
            }

...
public class Main {
    public static void main(String[] args) {
        ...
        for(int i=0;i<studioDatabase.length;i++){
            if(studioDatabase[i].numarFilme()>2){
                System.out.println(studioDatabase[i].getNumeStudio()+" has published more than 2 movies");
            }
            Film[] copyFilme=studioDatabase[i].getFilme();
            for(int j=0;j<copyFilme.length;j++){
                int ok=0;
                Actor[] copyActori=copyFilme[j].getActori();
                for(Actor actor:copyActori){
                    if(actor.getNumeActor()=="actor cu 2 oscaruri")
                        System.out.println(studioDatabase[i].getNumeStudio()+" plays the actor -actor cu 2 oscaruri-");
                    if(actor.getVarstaActor()>50)
                        ok=1;
                }
                if(ok==1)
                    System.out.println(copyFilme[j].getNumeFilm()+" plays at least an actor with age above 50");
            }
        }
